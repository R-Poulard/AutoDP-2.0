...
int get_G(pk_compound *pk,Node** first,Node** last,int * value,int d,int f,int k,int l);

int compute_G(pk_compound *pk,Node** first,Node** last,int d,int f,int k,int l);

void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int d,int f,int k,int l) ;

int get_F(pk_compound *pk,Node** first,Node** last,int * value,int c,int d,int h,int j);

int compute_F(pk_compound *pk,Node** first,Node** last,int c,int d,int h,int j);

void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int c,int d,int h,int j) ;

int get_E(pk_compound *pk,Node** first,Node** last,int * value,int c,int d,int h,int k);

int compute_E(pk_compound *pk,Node** first,Node** last,int c,int d,int h,int k);

void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int c,int d,int h,int k) ;

int get_D(pk_compound *pk,Node** first,Node** last,int * value,int b,int d,int h,int k);

int compute_D(pk_compound *pk,Node** first,Node** last,int b,int d,int h,int k);

void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int b,int d,int h,int k) ;

int get_C(pk_compound *pk,Node** first,Node** last,int * value,int a,int d,int g,int k);

int compute_C(pk_compound *pk,Node** first,Node** last,int a,int d,int g,int k);

void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int d,int g,int k) ;

int get_B(pk_compound *pk,Node** first,Node** last,int * value,int a,int d,int f,int k);

int compute_B(pk_compound *pk,Node** first,Node** last,int a,int d,int f,int k);

void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int d,int f,int k) ;
        ...
        switch(topTuple->values[0]){
            ...
            break;//Clique
             ...
             case 71:
                 ...
                 if(compute_G(pk,&add_on_first,&add_on_last,d,f,k,l)==1){
             ...
             case 70:
                 ...
                 if(compute_F(pk,&add_on_first,&add_on_last,c,d,h,j)==1){
             ...
             case 69:
                 ...
                 if(compute_E(pk,&add_on_first,&add_on_last,c,d,h,k)==1){
             ...
             case 68:
                 ...
                 if(compute_D(pk,&add_on_first,&add_on_last,b,d,h,k)==1){
             ...
             case 67:
                 ...
                 if(compute_C(pk,&add_on_first,&add_on_last,a,d,g,k)==1){
             ...
             case 66:
                 ...
                 if(compute_B(pk,&add_on_first,&add_on_last,a,d,f,k)==1){
...
int compute_E(pk_compound *pk,Node** first,Node** last,int c,int d,int h,int k) {
    ...
    for (int j=h+1;j<k+1;j++) {
      ...
      int tmp0;
      if(get_F( pk,first,last,&tmp0,c,d,h,j)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_D(pk_compound *pk,Node** first,Node** last,int b,int d,int h,int k) {
    ...
    for (int c=b;c<d;c++) {
      ...
      int tmp0;
      if(get_E( pk,first,last,&tmp0,c,d,h,k)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_C(pk_compound *pk,Node** first,Node** last,int a,int d,int g,int k) {
    ...
    for (int b=a+1;b<d;b++) {
        for (int h=g+1;h<k;h++) {
          ...
          int tmp0;
          if(get_D( pk,first,last,&tmp0,b,d,h,k)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
          int tmp1;
          if(get_CLIQUE0( pk,first,last,&tmp1,a,b-1,g,h-1)==0){possible=0;}
...
int compute_B(pk_compound *pk,Node** first,Node** last,int a,int d,int f,int k) {
    ...
    for (int g=f;g<k-1;g++) {
      ...
      int tmp0;
      if(get_C( pk,first,last,&tmp0,a,d,g,k)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
    ...
    for (int d=a+2;d<l-3;d++) {
        for (int f=d+1;f<l-2;f++) {
            for (int k=f+2;k<l;k++) {
              ...
              int tmp0;
              if(get_B( pk,first,last,&tmp0,a,d,f,k)==0){possible=0;}
              else if(tmp0==INT_MAX){continue;}
              int tmp1;
              if(get_G( pk,first,last,&tmp1,d,f,k,l)==0){possible=0;}
              else if(tmp1==INT_MAX){continue;}
...
void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int c,int d,int h,int k) {
    ...
    for (int j=h+1;j<k+1;j++) {
      ...
      int tmp0;
      get_F( pk,NULL,NULL,&tmp0,c,d,h,j);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,j,k-1);

        backtrace_F( pk,bt,tmp0,c,d,h,j);
...
void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int b,int d,int h,int k) {
    ...
    for (int c=b;c<d;c++) {
      ...
      int tmp0;
      get_E( pk,NULL,NULL,&tmp0,c,d,h,k);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,b,c-1);

        backtrace_E( pk,bt,tmp0,c,d,h,k);
...
void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int d,int g,int k) {
    ...
    for (int b=a+1;b<d;b++) {
        for (int h=g+1;h<k;h++) {
          ...
          int tmp0;
          get_D( pk,NULL,NULL,&tmp0,b,d,h,k);
          if(tmp0==INT_MAX){continue;}
          int tmp1;
          get_CLIQUE0( pk,NULL,NULL,&tmp1,a,b-1,g,h-1);
          if(tmp1==INT_MAX){continue;}

          if(score==add(add(tmp0,tmp1),0)){

            backtrace_D( pk,bt,tmp0,b,d,h,k);
            backtrace_CLIQUE0( pk,bt,tmp1,a,b-1,g,h-1);
            bt->bracket+=1;
...
void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int d,int f,int k) {
    ...
    for (int g=f;g<k-1;g++) {
      ...
      int tmp0;
      get_C( pk,NULL,NULL,&tmp0,a,d,g,k);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,f,g-1);

        backtrace_C( pk,bt,tmp0,a,d,g,k);
    ...
    for (int d=a+2;d<l-3;d++) {
        for (int f=d+1;f<l-2;f++) {
            for (int k=f+2;k<l;k++) {
              ...
              int tmp0;
              get_B( pk,NULL,NULL,&tmp0,a,d,f,k);
              if(tmp0==INT_MAX){continue;}
              int tmp1;
              get_G( pk,NULL,NULL,&tmp1,d,f,k,l);
              if(tmp1==INT_MAX){continue;}

              if(score==add(add(tmp0,tmp1),0)){

                backtrace_B( pk,bt,tmp0,a,d,f,k);
                backtrace_G( pk,bt,tmp1,d,f,k,l);
