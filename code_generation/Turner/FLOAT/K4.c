...
float compute_J(HashTable *hashTable,int a,int c,int i,int j) ;
...
float compute_J(HashTable *hashTable,int a,int c,int i,int j) {
    ...
    for (int b=a+1;b<c+1;b++) {
      ...
      float tmp0= compute_CLIQUE0( hashTable,a,b-1,i,j-1);
...
float compute_I(HashTable *hashTable,int a,int c,int h,int j) {
    ...
    for (int i=h;i<j;i++) {
      ...
      float tmp0= compute_J( hashTable,a,c,i,j);
      if(tmp0==FLT_MAX){continue;}
...
float compute_G(HashTable *hashTable,int c,int e,int j,int l) {
    ...
    for (int d=c+1;d<e+1;d++) {
      ...
      float tmp0= compute_H( hashTable,c,d,j,l);
      if(tmp0==FLT_MAX){continue;}
...
float compute_F(HashTable *hashTable,int c,int e,int j,int m) {
    ...
    for (int l=j+1;l<m+1;l++) {
      ...
      float tmp0= compute_G( hashTable,c,e,j,l);
      if(tmp0==FLT_MAX){continue;}
...
float compute_E(HashTable *hashTable,int a,int e,int h,int m) {
    ...
    for (int c=a+1;c<e;c++) {
        for (int j=h+1;j<m;j++) {
          ...
          float tmp0= compute_I( hashTable,a,c,h,j);
          if(tmp0==FLT_MAX){continue;}
          float tmp1= compute_F( hashTable,c,e,j,m);
...
float compute_D(HashTable *hashTable,int a,int f,int h,int n) {
    ...
    for (int e=a+2;e<f;e++) {
        for (int m=h+2;m<n;m++) {
          ...
          if(tmp0==FLT_MAX){continue;}
          float tmp1= compute_E( hashTable,a,e,h,m);
          if(tmp1==FLT_MAX){continue;}
...
float compute_C(HashTable *hashTable,int a,int f,int h,int o) {
    ...
    for (int n=h+3;n<o+1;n++) {
      ...
      float tmp0= compute_D( hashTable,a,f,h,n);
      if(tmp0==FLT_MAX){continue;}
...
float compute_B0(HashTable *hashTable,int g, int p, int a,int f){
    ...
    float min_value=add(INTB(g,p,g,p),compute_B1(hashTable,g+1,p-1,a,f));
...
float compute_B1(HashTable *hashTable,int g, int p, int a,int f) {
    ...
    float tmp0= compute_C( hashTable,a,f,g,p+1);
    if(tmp0==FLT_MAX){ goto loop;}

    min_value=add(tmp0,0);

    loop:
    for(int tmp1=g;tmp1<=min(p-1,FLT_MAX);tmp1++){
        for(int tmp2=max(FLT_MIN,tmp1+4);tmp2<=p;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                float tmp3=INTB(tmp1,tmp2,g,p);
...
float compute_A(HashTable *hashTable,int a,int p) {
    ...
    for (int f=a+3;f<p-4;f++) {
      ...
      float mfe0 = MFEFree(f,g-1);
          ...
          float tmp0= compute_B0( hashTable,g,p-1,a,f);
          if(tmp0==FLT_MAX){continue;}
...
void backtrace_B0(HashTable *hashTable, float score, int g, int p, int a,int f) {
    ...
    float tmp=compute_B1(hashTable,g+1,p-1,a,f);
    float sc=INTB(g,p,g,p);
    if(score==add(sc,tmp)){
    ...
    for(int tmp1=g;tmp1<=min(p-1,FLT_MAX);tmp1++){
        for(int tmp2=max(FLT_MIN,tmp1+4);tmp2<=p;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                float tmp3=INTB(tmp1,tmp2,g,p);
                if(tmp3!=INT_MAX){
                    float tmp0=compute_B1(hashTable,tmp1+1,tmp2-1,a,f);
