...
int compute_E(HashTable *hashTable,int c,int d,int h,int k) {
    ...
    for (int j=h+1;j<k+1;j++) {
      ...
      int tmp0= compute_F( hashTable,c,d,h,j);
      if(tmp0==INT_MAX){continue;}
...
int compute_D(HashTable *hashTable,int b,int d,int h,int k) {
    ...
    for (int c=b;c<d;c++) {
      ...
      int tmp0= compute_E( hashTable,c,d,h,k);
      if(tmp0==INT_MAX){continue;}
...
int compute_C(HashTable *hashTable,int a,int d,int g,int k) {
    ...
    for (int b=a+1;b<d;b++) {
        for (int h=g+1;h<k;h++) {
          ...
          int tmp0= compute_D( hashTable,b,d,h,k);
          if(tmp0==INT_MAX){continue;}
          int tmp1= compute_CLIQUE0( hashTable,a,b-1,g,h-1);
...
int compute_B(HashTable *hashTable,int a,int d,int f,int k) {
    ...
    for (int g=f;g<k-1;g++) {
      ...
      int tmp0= compute_C( hashTable,a,d,g,k);
      if(tmp0==INT_MAX){continue;}
    ...
    for (int d=a+2;d<l-3;d++) {
        for (int f=d+1;f<l-2;f++) {
            for (int k=f+2;k<l;k++) {
              ...
              int tmp0= compute_B( hashTable,a,d,f,k);
              if(tmp0==INT_MAX){continue;}
              int tmp1= compute_G( hashTable,d,f,k,l);
              if(tmp1==INT_MAX){continue;}
