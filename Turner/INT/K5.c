...
int compute_K(HashTable *hashTable,int i,int j,int r,int t) ;
...
int compute_K(HashTable *hashTable,int i,int j,int r,int t) {
    ...
    for (int s=r;s<t;s++) {
      ...
      int tmp0= compute_CLIQUE0( hashTable,i,j-1,s,t-1);
...
int compute_J0(HashTable *hashTable,int a, int l, int c,int j){
    ...
    int min_value=add(INTB(a,l,a,l),compute_J1(hashTable,a+1,l-1,c,j));
...
int compute_I0(HashTable *hashTable,int e, int p, int g,int n){
    ...
    int min_value=add(INTB(e,p,e,p),compute_I1(hashTable,e+1,p-1,g,n));
...
int compute_H(HashTable *hashTable,int d,int g,int n,int p) {
    ...
    for (int e=d;e<g;e++) {
      ...
      int tmp0= compute_I0( hashTable,e,p-1,g,n);
      if(tmp0==INT_MAX){continue;}
...
int compute_G(HashTable *hashTable,int d,int g,int n,int q) {
    ...
    for (int p=n+1;p<q+1;p++) {
      ...
      int tmp0= compute_H( hashTable,d,g,n,p);
      if(tmp0==INT_MAX){continue;}
...
int compute_F(HashTable *hashTable,int c,int d,int l,int n) {
    ...
    for (int m=l;m<n;m++) {
      ...
      int tmp0= compute_CLIQUE0( hashTable,c,d-1,m,n-1);
      if(tmp0==INT_MAX){continue;}
...
int compute_E(HashTable *hashTable,int c,int g,int l,int q) {
    ...
    for (int d=c+1;d<g;d++) {
        for (int n=l+1;n<q;n++) {
          ...
          int tmp0= compute_G( hashTable,d,g,n,q);
          if(tmp0==INT_MAX){continue;}
          int tmp1= compute_F( hashTable,c,d,l,n);
...
int compute_D(HashTable *hashTable,int a,int g,int j,int q) {
    ...
    for (int c=a+1;c<g-1;c++) {
        for (int l=j+1;l<q-1;l++) {
          ...
          int tmp0= compute_E( hashTable,c,g,l,q);
          if(tmp0==INT_MAX){continue;}
          int tmp1= compute_J0( hashTable,a,l-1,c,j);
          if(tmp1==INT_MAX){continue;}
...
int compute_C(HashTable *hashTable,int a,int h,int j,int r) {
    ...
    for (int g=a+3;g<h;g++) {
        for (int q=j+3;q<r;q++) {
          ...
          int tmp0= compute_D( hashTable,a,g,j,q);
          if(tmp0==INT_MAX){continue;}
          int tmp1= compute_CLIQUE0( hashTable,g,h-1,q,r-1);
          if(tmp1==INT_MAX){continue;}
...
int compute_B(HashTable *hashTable,int a,int i,int j,int r) {
    ...
    for (int h=a+4;h<i+1;h++) {
      ...
      int tmp0= compute_C( hashTable,a,h,j,r);
      if(tmp0==INT_MAX){continue;}
    ...
    for (int i=a+4;i<t-5;i++) {
        for (int j=i+1;j<t-4;j++) {
            for (int r=j+4;r<t;r++) {
              ...
              int tmp0= compute_B( hashTable,a,i,j,r);
              if(tmp0==INT_MAX){continue;}
              int tmp1= compute_K( hashTable,i,j,r,t);
              if(tmp1==INT_MAX){continue;}
...
void backtrace_J0(HashTable *hashTable, int score, int a, int l, int c,int j) {
    ...
    int tmp=compute_J1(hashTable,a+1,l-1,c,j);
    int sc=INTB(a,l,a,l);
    if(score==add(sc,tmp)){
    ...
    for(int tmp1=a;tmp1<=min(l-1,c-1);tmp1++){
        for(int tmp2=max(j,tmp1+5);tmp2<=l;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                int tmp3=INTB(tmp1,tmp2,a,l);
                if(tmp3!=INT_MAX){
                    int tmp0=compute_J1(hashTable,tmp1+1,tmp2-1,c,j);
...
void backtrace_I0(HashTable *hashTable, int score, int e, int p, int g,int n) {
    ...
    int tmp=compute_I1(hashTable,e+1,p-1,g,n);
    int sc=INTB(e,p,e,p);
    if(score==add(sc,tmp)){
    ...
    for(int tmp1=e;tmp1<=min(p-1,g-1);tmp1++){
        for(int tmp2=max(n,tmp1+5);tmp2<=p;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                int tmp3=INTB(tmp1,tmp2,e,p);
                if(tmp3!=INT_MAX){
                    int tmp0=compute_I1(hashTable,tmp1+1,tmp2-1,g,n);
                    if(score==add(tmp0,tmp3)){
