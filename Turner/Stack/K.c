...
int get_G(HashTable *hashTable,Node** first,Node** last,int * value,int f,int h,int k,int l);

int compute_G(HashTable *hashTable,Node** first,Node** last,int f,int h,int k,int l);

void backtrace_G(HashTable *hashTable,int score,int f,int h,int k,int l) ;

int get_F(HashTable *hashTable,Node** first,Node** last,int * value,int c,int e,int i,int j);

int compute_F(HashTable *hashTable,Node** first,Node** last,int c,int e,int i,int j);

void backtrace_F(HashTable *hashTable,int score,int c,int e,int i,int j) ;

int get_E(HashTable *hashTable,Node** first,Node** last,int * value,int b,int e,int i,int j);

int compute_E(HashTable *hashTable,Node** first,Node** last,int b,int e,int i,int j);

void backtrace_E(HashTable *hashTable,int score,int b,int e,int i,int j) ;

int get_D0(HashTable *hashTable,Node** first,Node** last,int * value,int a, int f, int i,int j);

int get_D1(HashTable *hashTable,Node** first,Node** last,int * value,int a, int f, int i,int j);

int compute_D1(HashTable *hashTable,Node** first,Node** last,int a, int f, int i,int j);
...
int get_C(HashTable *hashTable,Node** first,Node** last,int * value,int a,int f,int i,int k);

int compute_C(HashTable *hashTable,Node** first,Node** last,int a,int f,int i,int k);

void backtrace_C(HashTable *hashTable,int score,int a,int f,int i,int k) ;

int get_B(HashTable *hashTable,Node** first,Node** last,int * value,int a,int f,int h,int k);

int compute_B(HashTable *hashTable,Node** first,Node** last,int a,int f,int h,int k);
        ...
        switch(topTuple->values[0]){
            ...
            break;//Clique
             ...
             case 71:
                 ...
                 if(compute_G(hasTable,&add_on_first,&add_on_last,f,h,k,l)==1){
             ...
             case 70:
                 ...
                 if(compute_F(hasTable,&add_on_first,&add_on_last,c,e,i,j)==1){
             ...
             case 69:
                 ...
                 if(compute_E(hasTable,&add_on_first,&add_on_last,b,e,i,j)==1){
             ...
             case 68:
                 ...
                 if(compute_D1(hasTable,&add_on_first,&add_on_last,a,f,i,j)==1){
             ...
             case 67:
                 ...
                 if(compute_C(hasTable,&add_on_first,&add_on_last,a,f,i,k)==1){
             ...
             case 66:
                 ...
                 if(compute_B(hasTable,&add_on_first,&add_on_last,a,f,h,k)==1){
...
int compute_G(HashTable *hashTable,Node** first,Node** last,int f,int h,int k,int l) {
    ...
    for (int g=f;g<h;g++) {
      ...
      int tmp0;
      if(get_CLIQUE0( hashTable,first,last,&tmp0,g,h-1,k,l-1)==0){possible=0;}
...
int compute_E(HashTable *hashTable,Node** first,Node** last,int b,int e,int i,int j) {
    ...
    for (int c=b;c<e;c++) {
      ...
      int tmp0;
      if(get_F( hashTable,first,last,&tmp0,c,e,i,j)==0){possible=0;}
      if(tmp0==INT_MAX){continue;}
...
int compute_D1(HashTable *hashTable,Node** first,Node** last,int a, int f, int i,int j) {
    ...
    int tmp0;
    if(get_E( hashTable,first,last,&tmp0,a,f+1,i,j)==0){possible=0;}
    ...
    for(int tmp1=a;tmp1<=min(f-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+2);tmp2<=f;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                ...
                if(tmp3!=INT_MAX){
                    int tmp;
                    if(get_D1(hashTable,first,last,&tmp,tmp1+1,tmp2-1,i,j)==0){
                        possible=0;
                    }
                    if(possible==1){
                        //printf("C1 res= %d",tmp);
...
int compute_C(HashTable *hashTable,Node** first,Node** last,int a,int f,int i,int k) {
    ...
    for (int j=i+1;j<k+1;j++) {
      ...
      int tmp0;
      if(get_D0( hashTable,first,last,&tmp0,a,f-1,i,j)==0){possible=0;}
      if(tmp0==INT_MAX){continue;}
...
int compute_B(HashTable *hashTable,Node** first,Node** last,int a,int f,int h,int k) {
    ...
    for (int i=h;i<k;i++) {
      ...
      int tmp0;
      if(get_C( hashTable,first,last,&tmp0,a,f,i,k)==0){possible=0;}
      if(tmp0==INT_MAX){continue;}
    ...
    for (int f=a+3;f<l-2;f++) {
        for (int h=f+1;h<l-1;h++) {
            for (int k=h+1;k<l;k++) {
              ...
              int tmp0;
              if(get_B( hashTable,first,last,&tmp0,a,f,h,k)==0){possible=0;}
              if(tmp0==INT_MAX){continue;}
              int tmp1;
              if(get_G( hashTable,first,last,&tmp1,f,h,k,l)==0){possible=0;}
...
void backtrace_G(HashTable *hashTable,int score,int f,int h,int k,int l) {
    ...
    for (int g=f;g<h;g++) {
      ...
      int tmp0;
      get_CLIQUE0( hashTable,NULL,NULL,&tmp0,g,h-1,k,l-1);
      if(tmp0==INT_MAX){continue;}
...
void backtrace_E(HashTable *hashTable,int score,int b,int e,int i,int j) {
    ...
    for (int c=b;c<e;c++) {
      ...
      int tmp0;
      get_F( hashTable,NULL,NULL,&tmp0,c,e,i,j);
      if(tmp0==INT_MAX){continue;}
...
void backtrace_D0(HashTable *hashTable, int score, int a, int f, int i,int j) {
    ...
    int tmp;
    get_D1(hashTable,NULL,NULL,&tmp,a+1,f-1,i,j);
    int sc=INTB(a,f,-1,-1);
    if(score==add(sc,tmp)){
...
void backtrace_D1(HashTable *hashTable, int score, int a, int f, int i,int j) {
    ...
    int tmp0;
    get_E( hashTable,NULL,NULL,&tmp0,a,f+1,i,j);
     if(tmp0==INT_MAX){ goto loop;}
    ...
    for(int tmp1=a;tmp1<=min(f-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+2);tmp2<=f;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                ...
                if(tmp3!=INT_MAX){
                    int tmp0;
                    get_D1(hashTable,NULL,NULL,&tmp0,tmp1+1,tmp2-1,i,j);
...
void backtrace_C(HashTable *hashTable,int score,int a,int f,int i,int k) {
    ...
    for (int j=i+1;j<k+1;j++) {
      ...
      int tmp0;
      get_D0( hashTable,NULL,NULL,&tmp0,a,f-1,i,j);
      if(tmp0==INT_MAX){continue;}
...
void backtrace_B(HashTable *hashTable,int score,int a,int f,int h,int k) {
    ...
    for (int i=h;i<k;i++) {
      ...
      int tmp0;
      get_C( hashTable,NULL,NULL,&tmp0,a,f,i,k);
      if(tmp0==INT_MAX){continue;}
    ...
    for (int f=a+3;f<l-2;f++) {
        for (int h=f+1;h<l-1;h++) {
            for (int k=h+1;k<l;k++) {
              ...
              int tmp0;
              get_B( hashTable,NULL,NULL,&tmp0,a,f,h,k);
              if(tmp0==INT_MAX){continue;}
              int tmp1;
              get_G( hashTable,NULL,NULL,&tmp1,f,h,k,l);
              if(tmp1==INT_MAX){continue;}
