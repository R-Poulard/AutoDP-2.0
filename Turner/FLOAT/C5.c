...
float compute_M(HashTable *hashTable,int m,int n,int r,int t) ;
...
float compute_M(HashTable *hashTable,int m,int n,int r,int t) {
    ...
    for (int s=r;s<t;s++) {
      ...
      float tmp0= compute_CLIQUE0( hashTable,m,n-1,s,t-1);
...
float compute_K(HashTable *hashTable,int i,int k,int n,int p) {
    ...
    for (int o=n;o<p;o++) {
      ...
      float mfe0 = MFEFree(n,o-1);

      float tmp0= compute_L( hashTable,i,k,o,p);
      if(tmp0==FLT_MAX){continue;}
...
float compute_J(HashTable *hashTable,int a,int h,int k,int n,int p) {
    ...
    for (int i=h;i<k;i++) {
      ...
      float tmp0= compute_K( hashTable,i,k,n,p);
      if(tmp0==FLT_MAX){continue;}
...
float compute_H(HashTable *hashTable,int c,int e,int q,int r) {
    ...
    for (int d=c+1;d<e+1;d++) {
      ...
      float tmp0= compute_CLIQUE0( hashTable,c,d-1,q,r-1);
      if(tmp0==FLT_MAX){continue;}
...
float compute_G(HashTable *hashTable,int c,int e,int p,int r) {
    ...
    for (int q=p;q<r;q++) {
      ...
      float tmp0= compute_H( hashTable,c,e,q,r);
      if(tmp0==FLT_MAX){continue;}
...
float compute_F(HashTable *hashTable,int b,int e,int p,int r) {
    ...
    for (int c=b;c<e;c++) {
      ...
      float tmp0= compute_G( hashTable,c,e,p,r);
      if(tmp0==FLT_MAX){continue;}
...
float compute_E(HashTable *hashTable,int b,int g,int k,int l,int p,int r) {
    ...
    for (int e=b+1;e<g;e++) {
      ...
      float tmp0= compute_F( hashTable,b,e,p,r);
      if(tmp0==FLT_MAX){continue;}
      float tmp1= compute_I( hashTable,b,e,g,k,l);
...
float compute_D0(HashTable *hashTable,int a, int h, int k,int l,int p,int r){
    ...
    float min_value=add(INTB(a,h,a,h),compute_D1(hashTable,a+1,h-1,k,l,p,r));
...
float compute_D1(HashTable *hashTable,int a, int h, int k,int l,int p,int r) {
    ...
    float tmp0= compute_E( hashTable,a,h+1,k,l,p,r);
    if(tmp0==FLT_MAX){ goto loop;}

    min_value=add(tmp0,0);

    loop:
    for(int tmp1=a;tmp1<=min(h-1,FLT_MAX);tmp1++){
        for(int tmp2=max(FLT_MIN,tmp1+3);tmp2<=h;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                float tmp3=INTB(tmp1,tmp2,a,h);
    ...
    for (int h=a+4;h<l-1;h++) {
        for (int k=h+1;k<l;k++) {
            for (int p=n+1;p<r;p++) {
              ...
              float tmp0= compute_D0( hashTable,a,h-1,k,l,p,r);
              if(tmp0==FLT_MAX){continue;}
              float tmp1= compute_J( hashTable,a,h,k,n,p);
              if(tmp1==FLT_MAX){continue;}
...
float compute_B(HashTable *hashTable,int a,int m,int n,int r) {
    ...
    for (int l=a+6;l<m+1;l++) {
      ...
      float tmp0= compute_C( hashTable,a,l,n,r);
      if(tmp0==FLT_MAX){continue;}
    ...
    for (int m=a+6;m<t-3;m++) {
        for (int n=m+1;n<t-2;n++) {
            for (int r=n+2;r<t;r++) {
              ...
              float tmp0= compute_B( hashTable,a,m,n,r);
              if(tmp0==FLT_MAX){continue;}
              float tmp1= compute_M( hashTable,m,n,r,t);
              if(tmp1==FLT_MAX){continue;}
...
void backtrace_D0(HashTable *hashTable, float score, int a, int h, int k,int l,int p,int r) {
    ...
    float tmp=compute_D1(hashTable,a+1,h-1,k,l,p,r);
    float sc=INTB(a,h,a,h);
    if(score==add(sc,tmp)){
    ...
    for(int tmp1=a;tmp1<=min(h-1,FLT_MAX);tmp1++){
        for(int tmp2=max(FLT_MIN,tmp1+3);tmp2<=h;tmp2++){
            if(evaluate(tmp1,tmp2) && tmp1-tmp2+1<=THETA){
                float tmp3=INTB(tmp1,tmp2,a,h);
                if(tmp3!=INT_MAX){
                    float tmp0=compute_D1(hashTable,tmp1+1,tmp2-1,k,l,p,r);
