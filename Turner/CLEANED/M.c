...
int get_I(pk_compound *pk,Node** first,Node** last,int * value,int i,int j,int n,int p);

int compute_I(pk_compound *pk,Node** first,Node** last,int i,int j,int n,int p);

void backtrace_I(pk_compound *pk,bt_struct* bt,int score,int i,int j,int n,int p) ;

int get_H(pk_compound *pk,Node** first,Node** last,int * value,int a,int b,int g,int i);

int compute_H(pk_compound *pk,Node** first,Node** last,int a,int b,int g,int i);

void backtrace_H(pk_compound *pk,bt_struct* bt,int score,int a,int b,int g,int i) ;

int get_G(pk_compound *pk,Node** first,Node** last,int * value,int e,int f,int l,int n);

int compute_G(pk_compound *pk,Node** first,Node** last,int e,int f,int l,int n);

void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int e,int f,int l,int n) ;

int get_F(pk_compound *pk,Node** first,Node** last,int * value,int d,int f,int l,int n);

int compute_F(pk_compound *pk,Node** first,Node** last,int d,int f,int l,int n);

void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int d,int f,int l,int n) ;

int get_E(pk_compound *pk,Node** first,Node** last,int * value,int c,int d,int j,int l);

int compute_E(pk_compound *pk,Node** first,Node** last,int c,int d,int j,int l);

void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int c,int d,int j,int l) ;

int get_D(pk_compound *pk,Node** first,Node** last,int * value,int c,int f,int j,int n);

int compute_D(pk_compound *pk,Node** first,Node** last,int c,int f,int j,int n);

void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int c,int f,int j,int n) ;

int get_C(pk_compound *pk,Node** first,Node** last,int * value,int b,int g,int j,int n);

int compute_C(pk_compound *pk,Node** first,Node** last,int b,int g,int j,int n);

void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int b,int g,int j,int n) ;

int get_B(pk_compound *pk,Node** first,Node** last,int * value,int a,int i,int j,int n);

int compute_B(pk_compound *pk,Node** first,Node** last,int a,int i,int j,int n);

void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int i,int j,int n) ;
        ...
        switch(topTuple->values[0]){
            ...
            break;//Clique
             ...
             case 73:
                 ...
                 if(compute_I(pk,&add_on_first,&add_on_last,i,j,n,p)==1){
             ...
             case 72:
                 ...
                 if(compute_H(pk,&add_on_first,&add_on_last,a,b,g,i)==1){
             ...
             case 71:
                 ...
                 if(compute_G(pk,&add_on_first,&add_on_last,e,f,l,n)==1){
             ...
             case 70:
                 ...
                 if(compute_F(pk,&add_on_first,&add_on_last,d,f,l,n)==1){
             ...
             case 69:
                 ...
                 if(compute_E(pk,&add_on_first,&add_on_last,c,d,j,l)==1){
             ...
             case 68:
                 ...
                 if(compute_D(pk,&add_on_first,&add_on_last,c,f,j,n)==1){
             ...
             case 67:
                 ...
                 if(compute_C(pk,&add_on_first,&add_on_last,b,g,j,n)==1){
             ...
             case 66:
                 ...
                 if(compute_B(pk,&add_on_first,&add_on_last,a,i,j,n)==1){
...
int compute_H(pk_compound *pk,Node** first,Node** last,int a,int b,int g,int i) {
    ...
    for (int h=g+1;h<i+1;h++) {
      if(!evaluate(pk,a,h-1)||!evaluate(pk,b-1,g)){continue;}
      int mfe0 = MFEFree(pk,h,i-1);

      int tmp0;
      if(get_CLIQUE0( pk,first,last,&tmp0,a,b-1,g,h-1)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_F(pk_compound *pk,Node** first,Node** last,int d,int f,int l,int n) {
    ...
    for (int e=d;e<f;e++) {
      ...
      int tmp0;
      if(get_G( pk,first,last,&tmp0,e,f,l,n)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_D(pk_compound *pk,Node** first,Node** last,int c,int f,int j,int n) {
    ...
    for (int d=c+1;d<f;d++) {
        for (int l=j+1;l<n;l++) {
          ...
          int tmp0;
          if(get_F( pk,first,last,&tmp0,d,f,l,n)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
          int tmp1;
          if(get_E( pk,first,last,&tmp1,c,d,j,l)==0){possible=0;}
...
int compute_C(pk_compound *pk,Node** first,Node** last,int b,int g,int j,int n) {
    ...
    for (int c=b;c<g-1;c++) {
      ...
      int mfe0 = MFEFree(pk,b,c-1);
              int mfe1 = MFEFree(pk,f,g-1);

          int tmp0;
          if(get_D( pk,first,last,&tmp0,c,f,j,n)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
...
int compute_B(pk_compound *pk,Node** first,Node** last,int a,int i,int j,int n) {
    ...
    for (int b=a+1;b<i-2;b++) {
        for (int g=b+2;g<i;g++) {
          ...
          int tmp0;
          if(get_C( pk,first,last,&tmp0,b,g,j,n)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
          int tmp1;
          if(get_H( pk,first,last,&tmp1,a,b,g,i)==0){possible=0;}
          else if(tmp1==INT_MAX){continue;}
    ...
    for (int i=a+4;i<p-3;i++) {
        for (int j=i+1;j<p-2;j++) {
            for (int n=j+2;n<p;n++) {
              ...
              int tmp0;
              if(get_B( pk,first,last,&tmp0,a,i,j,n)==0){possible=0;}
              else if(tmp0==INT_MAX){continue;}
              int tmp1;
              if(get_I( pk,first,last,&tmp1,i,j,n,p)==0){possible=0;}
              else if(tmp1==INT_MAX){continue;}
...
void backtrace_H(pk_compound *pk,bt_struct* bt,int score,int a,int b,int g,int i) {
    ...
    for (int h=g+1;h<i+1;h++) {
      ...
      int tmp0;
      get_CLIQUE0( pk,NULL,NULL,&tmp0,a,b-1,g,h-1);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,h,i-1);

        backtrace_CLIQUE0( pk,bt,tmp0,a,b-1,g,h-1);
        bt->bracket+=1;
...
void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int d,int f,int l,int n) {
    ...
    for (int e=d;e<f;e++) {
      ...
      int tmp0;
      get_G( pk,NULL,NULL,&tmp0,e,f,l,n);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,d,e-1);

        backtrace_G( pk,bt,tmp0,e,f,l,n);
...
void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int c,int f,int j,int n) {
    ...
    for (int d=c+1;d<f;d++) {
        for (int l=j+1;l<n;l++) {
          ...
          int tmp0;
          get_F( pk,NULL,NULL,&tmp0,d,f,l,n);
          if(tmp0==INT_MAX){continue;}
          int tmp1;
          get_E( pk,NULL,NULL,&tmp1,c,d,j,l);
          if(tmp1==INT_MAX){continue;}

          if(score==add(add(tmp0,tmp1),0)){

            backtrace_F( pk,bt,tmp0,d,f,l,n);
            backtrace_E( pk,bt,tmp1,c,d,j,l);
...
void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int b,int g,int j,int n) {
    ...
    for (int c=b;c<g-1;c++) {
      ...
      int mfe0 = MFEFree(pk,b,c-1);
          ...
          int tmp0;
          get_D( pk,NULL,NULL,&tmp0,c,f,j,n);
          if(tmp0==INT_MAX){continue;}

          if(score==add(tmp0,add(mfe0,mfe1))){
        backtrace_MFEFree(pk,bt,mfe0,b,c-1);
                backtrace_MFEFree(pk,bt,mfe1,f,g-1);

            backtrace_D( pk,bt,tmp0,c,f,j,n);
...
void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int i,int j,int n) {
    ...
    for (int b=a+1;b<i-2;b++) {
        for (int g=b+2;g<i;g++) {
          ...
          int tmp0;
          get_C( pk,NULL,NULL,&tmp0,b,g,j,n);
          if(tmp0==INT_MAX){continue;}
          int tmp1;
          get_H( pk,NULL,NULL,&tmp1,a,b,g,i);
          if(tmp1==INT_MAX){continue;}

          if(score==add(add(tmp0,tmp1),0)){

            backtrace_C( pk,bt,tmp0,b,g,j,n);
            backtrace_H( pk,bt,tmp1,a,b,g,i);
    ...
    for (int i=a+4;i<p-3;i++) {
        for (int j=i+1;j<p-2;j++) {
            for (int n=j+2;n<p;n++) {
              ...
              int tmp0;
              get_B( pk,NULL,NULL,&tmp0,a,i,j,n);
              if(tmp0==INT_MAX){continue;}
              int tmp1;
              get_I( pk,NULL,NULL,&tmp1,i,j,n,p);
              if(tmp1==INT_MAX){continue;}

              if(score==add(add(tmp0,tmp1),0)){

                backtrace_B( pk,bt,tmp0,a,i,j,n);
                backtrace_I( pk,bt,tmp1,i,j,n,p);
