...
int get_J(pk_compound *pk,Node** first,Node** last,int * value,int a,int c,int i,int j);

int compute_J(pk_compound *pk,Node** first,Node** last,int a,int c,int i,int j);

void backtrace_J(pk_compound *pk,bt_struct* bt,int score,int a,int c,int i,int j) ;

int get_I(pk_compound *pk,Node** first,Node** last,int * value,int a,int c,int h,int j);

int compute_I(pk_compound *pk,Node** first,Node** last,int a,int c,int h,int j);

void backtrace_I(pk_compound *pk,bt_struct* bt,int score,int a,int c,int h,int j) ;

int get_H(pk_compound *pk,Node** first,Node** last,int * value,int c,int d,int j,int l);

int compute_H(pk_compound *pk,Node** first,Node** last,int c,int d,int j,int l);

void backtrace_H(pk_compound *pk,bt_struct* bt,int score,int c,int d,int j,int l) ;

int get_G(pk_compound *pk,Node** first,Node** last,int * value,int c,int e,int j,int l);

int compute_G(pk_compound *pk,Node** first,Node** last,int c,int e,int j,int l);

void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int c,int e,int j,int l) ;

int get_F(pk_compound *pk,Node** first,Node** last,int * value,int c,int e,int j,int m);

int compute_F(pk_compound *pk,Node** first,Node** last,int c,int e,int j,int m);

void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int c,int e,int j,int m) ;

int get_E(pk_compound *pk,Node** first,Node** last,int * value,int a,int e,int h,int m);

int compute_E(pk_compound *pk,Node** first,Node** last,int a,int e,int h,int m);

void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int a,int e,int h,int m) ;

int get_D(pk_compound *pk,Node** first,Node** last,int * value,int a,int f,int h,int n);

int compute_D(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int n);

void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int n) ;

int get_C(pk_compound *pk,Node** first,Node** last,int * value,int a,int f,int h,int o);

int compute_C(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int o);

void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int o) ;

int get_B0(pk_compound* pk,Node** first,Node** last,int * value,int g, int p, int a,int f);

int get_B1(pk_compound* pk,Node** first,Node** last,int * value,int g, int p, int a,int f);

int compute_B1(pk_compound* pk,Node** first,Node** last,int g, int p, int a,int f);

void backtrace_B0(pk_compound *pk,bt_struct* bt, int score, int g, int p, int a,int f) ;

void backtrace_B1(pk_compound *pk,bt_struct* bt, int score, int g, int p, int a,int f) ;

int get_A(pk_compound *pk,Node** first,Node** last,int * value,int a,int p);

int compute_A(pk_compound *pk,Node** first,Node** last,int a,int p);

void backtrace_A(pk_compound *pk,bt_struct* bt,int score,int a,int p) ;


int init_root(pk_compound* pk){  
    int A = 65;
    int size = 3;
    for(int p=0+7;p<=pk->MAX;p++){
        for(int a=0;a<p-7;a++){
            int tab[]={A,a,p};
            Node* newNode = (Node*)malloc(sizeof(Node));
            newNode->data = createTuple(tab,size);
...
int compute_root(pk_compound* pk) {
    ...
    for(int p=0+7;p<=pk->MAX;p++){
        for(int a=0;a<p-7;a++){
            ...
            int mfe2=MFEFree(pk,p,pk->MAX-1);
            int mfe=add(mfe1,mfe2);
            int tmp;
            get_A(pk,NULL,NULL,&tmp,a,p);
        ...
        switch(topTuple->values[0]){
            ...
            break;//Clique
             ...
             case 74:
                 ...
                 if(compute_J(pk,&add_on_first,&add_on_last,a,c,i,j)==1){
             ...
             case 73:
                 ...
                 if(compute_I(pk,&add_on_first,&add_on_last,a,c,h,j)==1){
             ...
             case 72:
                 ...
                 if(compute_H(pk,&add_on_first,&add_on_last,c,d,j,l)==1){
             ...
             case 71:
                 ...
                 if(compute_G(pk,&add_on_first,&add_on_last,c,e,j,l)==1){
             ...
             case 70:
                 ...
                 if(compute_F(pk,&add_on_first,&add_on_last,c,e,j,m)==1){
             ...
             case 69:
                 ...
                 if(compute_E(pk,&add_on_first,&add_on_last,a,e,h,m)==1){
             ...
             case 68:
                 ...
                 if(compute_D(pk,&add_on_first,&add_on_last,a,f,h,n)==1){
             ...
             case 67:
                 ...
                 if(compute_C(pk,&add_on_first,&add_on_last,a,f,h,o)==1){
             ...
             case 66:
                 ...
                 if(compute_B1(pk,&add_on_first,&add_on_last,g,p,a,f)==1){
             ...
             case 65:
                 ...
                 if(compute_A(pk,&add_on_first,&add_on_last,a,p)==1){
...
void backtrace(pk_compound *pk,bt_struct* bt,int score) {
    ...
    for(int p=0+7;p<=pk->MAX;p++){
        for(int a=0;a<p-7;a++){
            ...
            int mfe=add(mfe1,mfe2);
            int tmp0;
            get_A(pk,NULL,NULL,&tmp0,a,p);
            if(score==add(mfe,tmp0)){
                backtrace_A(pk,bt,tmp0,a,p);
                backtrace_MFEFree(pk,bt,mfe1,0,a-1);
                backtrace_MFEFree(pk,bt,mfe2,p,pk->MAX-1);
                return;
            }
       }
    }
...
int compute_J(pk_compound *pk,Node** first,Node** last,int a,int c,int i,int j) {
    ...
    for (int b=a+1;b<c+1;b++) {
      if(!evaluate(pk,a,j-1)||!evaluate(pk,b-1,i)){continue;}
      int mfe0 = MFEFree(pk,b,c-1);

      int tmp0;
      if(get_CLIQUE0( pk,first,last,&tmp0,a,b-1,i,j-1)==0){possible=0;}
...
int compute_I(pk_compound *pk,Node** first,Node** last,int a,int c,int h,int j) {
    ...
    for (int i=h;i<j;i++) {
      ...
      int tmp0;
      if(get_J( pk,first,last,&tmp0,a,c,i,j)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_H(pk_compound *pk,Node** first,Node** last,int c,int d,int j,int l) {
    ...
    int possible=1;
    for (int k=j;k<l;k++) {
      if(!evaluate(pk,c,l-1)||!evaluate(pk,d-1,k)){continue;}
      int mfe0 = MFEFree(pk,j,k-1);

      int tmp0;
      if(get_CLIQUE0( pk,first,last,&tmp0,c,d-1,k,l-1)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_G(pk_compound *pk,Node** first,Node** last,int c,int e,int j,int l) {
    ...
    for (int d=c+1;d<e+1;d++) {
      ...
      int tmp0;
      if(get_H( pk,first,last,&tmp0,c,d,j,l)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_F(pk_compound *pk,Node** first,Node** last,int c,int e,int j,int m) {
    ...
    for (int l=j+1;l<m+1;l++) {
      ...
      int tmp0;
      if(get_G( pk,first,last,&tmp0,c,e,j,l)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_E(pk_compound *pk,Node** first,Node** last,int a,int e,int h,int m) {
    ...
    for (int c=a+1;c<e;c++) {
        for (int j=h+1;j<m;j++) {
          ...
          int tmp0;
          if(get_I( pk,first,last,&tmp0,a,c,h,j)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
          int tmp1;
          if(get_F( pk,first,last,&tmp1,c,e,j,m)==0){possible=0;}
...
int compute_D(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int n) {
    ...
    for (int e=a+2;e<f;e++) {
        for (int m=h+2;m<n;m++) {
          if(!evaluate(pk,e,n-1)||!evaluate(pk,f-1,m)){continue;}

          int tmp0;
          if(get_CLIQUE0( pk,first,last,&tmp0,e,f-1,m,n-1)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
          int tmp1;
          if(get_E( pk,first,last,&tmp1,a,e,h,m)==0){possible=0;}
          else if(tmp1==INT_MAX){continue;}
...
int compute_C(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int o) {
    ...
    for (int n=h+3;n<o+1;n++) {
      ...
      int tmp0;
      if(get_D( pk,first,last,&tmp0,a,f,h,n)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int get_B0(pk_compound* pk,Node** first,Node** last,int * value,int g, int p, int a,int f){
    ...
    if(g<0 || p<0 || p>=pk->MAX || g>=pk->MAX || g>p){
        *value=INT_MAX;
        return 1;
    }   
    if (get(pk->hashtable,tab,size,value)){
        *value=add(INTB(pk,g,p,-1,-1),*value);
        return 1;
    }
    else{
        Node* newNode = (Node*)malloc(sizeof(Node));
...
int compute_B1(pk_compound *pk,Node** first,Node** last,int g, int p, int a,int f) {
    ...
    int tmp0;
    if(get_C( pk,first,last,&tmp0,a,f,g,p+1)==0){possible=0;}
    ...
    for(int tmp1=g;tmp1<=min(p-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+4);tmp2<=p;tmp2++){
            if(evaluate(pk,tmp1,tmp2) && tmp1-tmp2+1<=pk->THETA){
                int tmp3=INTB(pk,tmp1,tmp2,g,p);

                if(tmp3!=INT_MAX){
                    int tmp;
                    if(get_B1(pk,first,last,&tmp,tmp1+1,tmp2-1,a,f)==0){
                        possible=0;
                    }
                    if(possible==1){
                        //printf("C1 res= %d",tmp);
...
int compute_A(pk_compound *pk,Node** first,Node** last,int a,int p) {
    ...
    for (int f=a+3;f<p-4;f++) {
        for (int g=f;g<p-4;g++) {
          if(!evaluate(pk,g,p-1)){continue;}
      int mfe0 = MFEFree(pk,f,g-1);

          int tmp0;
          if(get_B0( pk,first,last,&tmp0,g,p-1,a,f)==0){possible=0;}
          else if(tmp0==INT_MAX){continue;}
...
void backtrace_J(pk_compound *pk,bt_struct* bt,int score,int a,int c,int i,int j) {
    ...
    for (int b=a+1;b<c+1;b++) {
      ...
      int tmp0;
      get_CLIQUE0( pk,NULL,NULL,&tmp0,a,b-1,i,j-1);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,b,c-1);

        backtrace_CLIQUE0( pk,bt,tmp0,a,b-1,i,j-1);
        bt->bracket+=1;
...
void backtrace_I(pk_compound *pk,bt_struct* bt,int score,int a,int c,int h,int j) {
    ...
    for (int i=h;i<j;i++) {
      ...
      int tmp0;
      get_J( pk,NULL,NULL,&tmp0,a,c,i,j);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,h,i-1);

        backtrace_J( pk,bt,tmp0,a,c,i,j);
...
void backtrace_H(pk_compound *pk,bt_struct* bt,int score,int c,int d,int j,int l) {
    ...
    for (int k=j;k<l;k++) {
      ...
      int tmp0;
      get_CLIQUE0( pk,NULL,NULL,&tmp0,c,d-1,k,l-1);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,j,k-1);

        backtrace_CLIQUE0( pk,bt,tmp0,c,d-1,k,l-1);
        bt->bracket+=1;
...
void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int c,int e,int j,int l) {
    ...
    for (int d=c+1;d<e+1;d++) {
      ...
      int tmp0;
      get_H( pk,NULL,NULL,&tmp0,c,d,j,l);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,d,e-1);

        backtrace_H( pk,bt,tmp0,c,d,j,l);
...
void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int c,int e,int j,int m) {
    ...
    for (int l=j+1;l<m+1;l++) {
      ...
      int tmp0;
      get_G( pk,NULL,NULL,&tmp0,c,e,j,l);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,l,m-1);

        backtrace_G( pk,bt,tmp0,c,e,j,l);
...
void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int a,int e,int h,int m) {
    ...
    for (int c=a+1;c<e;c++) {
        for (int j=h+1;j<m;j++) {
          ...
          int tmp0;
          get_I( pk,NULL,NULL,&tmp0,a,c,h,j);
          if(tmp0==INT_MAX){continue;}
          int tmp1;
          get_F( pk,NULL,NULL,&tmp1,c,e,j,m);
          if(tmp1==INT_MAX){continue;}

          if(score==add(add(tmp0,tmp1),0)){

            backtrace_I( pk,bt,tmp0,a,c,h,j);
            backtrace_F( pk,bt,tmp1,c,e,j,m);
...
void backtrace_D(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int n) {
    ...
    for (int e=a+2;e<f;e++) {
        for (int m=h+2;m<n;m++) {
          ...
          int tmp0;
          get_CLIQUE0( pk,NULL,NULL,&tmp0,e,f-1,m,n-1);
          if(tmp0==INT_MAX){continue;}
          int tmp1;
          get_E( pk,NULL,NULL,&tmp1,a,e,h,m);
          if(tmp1==INT_MAX){continue;}

          if(score==add(add(tmp0,tmp1),0)){

            backtrace_CLIQUE0( pk,bt,tmp0,e,f-1,m,n-1);
            bt->bracket+=1;
            backtrace_E( pk,bt,tmp1,a,e,h,m);
...
void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int o) {
    ...
    for (int n=h+3;n<o+1;n++) {
      ...
      int tmp0;
      get_D( pk,NULL,NULL,&tmp0,a,f,h,n);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,n,o-1);

        backtrace_D( pk,bt,tmp0,a,f,h,n);
...
void backtrace_B0(pk_compound *pk,bt_struct* bt, int score, int g, int p, int a,int f) {
    ...
    int tmp;
    get_B1(pk,NULL,NULL,&tmp,g+1,p-1,a,f);
    int sc=INTB(pk,g,p,-1,-1);
    if(score==add(sc,tmp)){
        add_pairing(pk,bt,sc,g,p,-1,-1);
        backtrace_B1(pk,bt,tmp,g+1,p-1,a,f);
    }
    return;
}
void backtrace_B1(pk_compound *pk,bt_struct* bt, int score, int g, int p, int a,int f) {
    ...
    int tmp0;
    get_C( pk,NULL,NULL,&tmp0,a,f,g,p+1);
     if(tmp0==INT_MAX){ goto loop;}


    if(score==add(tmp0,0)){
               bt->bracket+=1;
backtrace_C( pk,bt,tmp0,a,f,g,p+1);
    ...
    for(int tmp1=g;tmp1<=min(p-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+4);tmp2<=p;tmp2++){
            if(evaluate(pk,tmp1,tmp2) && tmp1-tmp2+1<=pk->THETA){
                ...
                if(tmp3!=INT_MAX){
                    int tmp0;
                    get_B1(pk,NULL,NULL,&tmp0,tmp1+1,tmp2-1,a,f);
                    if(score==add(tmp0,tmp3)){
                        add_pairing(pk,bt,tmp3,tmp1,tmp2,g,p);
                        backtrace_B1(pk,bt,tmp0,tmp1+1,tmp2-1,a,f);
                        return;
                    }
                }
            }
...
void backtrace_A(pk_compound *pk,bt_struct* bt,int score,int a,int p) {
    ...
    for (int f=a+3;f<p-4;f++) {
      ...
      int mfe0 = MFEFree(pk,f,g-1);
          ...
          int tmp0;
          get_B0( pk,NULL,NULL,&tmp0,g,p-1,a,f);
          if(tmp0==INT_MAX){continue;}

          if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,f,g-1);

           backtrace_B0( pk,bt,tmp0,g,p-1,a,f);
