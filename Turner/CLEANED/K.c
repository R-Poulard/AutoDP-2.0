...
int get_G(pk_compound *pk,Node** first,Node** last,int * value,int f,int h,int k,int l);

int compute_G(pk_compound *pk,Node** first,Node** last,int f,int h,int k,int l);

void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int f,int h,int k,int l) ;

int get_F(pk_compound *pk,Node** first,Node** last,int * value,int c,int e,int i,int j);

int compute_F(pk_compound *pk,Node** first,Node** last,int c,int e,int i,int j);

void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int c,int e,int i,int j) ;

int get_E(pk_compound *pk,Node** first,Node** last,int * value,int b,int e,int i,int j);

int compute_E(pk_compound *pk,Node** first,Node** last,int b,int e,int i,int j);

void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int b,int e,int i,int j) ;

int get_D0(pk_compound* pk,Node** first,Node** last,int * value,int a, int f, int i,int j);

int get_D1(pk_compound* pk,Node** first,Node** last,int * value,int a, int f, int i,int j);

int compute_D1(pk_compound* pk,Node** first,Node** last,int a, int f, int i,int j);

void backtrace_D0(pk_compound *pk,bt_struct* bt, int score, int a, int f, int i,int j) ;

void backtrace_D1(pk_compound *pk,bt_struct* bt, int score, int a, int f, int i,int j) ;

int get_C(pk_compound *pk,Node** first,Node** last,int * value,int a,int f,int i,int k);

int compute_C(pk_compound *pk,Node** first,Node** last,int a,int f,int i,int k);

void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int f,int i,int k) ;

int get_B(pk_compound *pk,Node** first,Node** last,int * value,int a,int f,int h,int k);

int compute_B(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int k);

void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int k) ;

int get_A(pk_compound *pk,Node** first,Node** last,int * value,int a,int l);

int compute_A(pk_compound *pk,Node** first,Node** last,int a,int l);

void backtrace_A(pk_compound *pk,bt_struct* bt,int score,int a,int l) ;


int init_root(pk_compound* pk){  
    int A = 65;
    int size = 3;
    for(int l=0+5;l<=pk->MAX;l++){
        for(int a=0;a<l-5;a++){
            int tab[]={A,a,l};
            Node* newNode = (Node*)malloc(sizeof(Node));
            newNode->data = createTuple(tab,size);
...
int compute_root(pk_compound* pk) {
    ...
    for(int l=0+5;l<=pk->MAX;l++){
        for(int a=0;a<l-5;a++){
            ...
            int mfe2=MFEFree(pk,l,pk->MAX-1);
            int mfe=add(mfe1,mfe2);
            int tmp;
            get_A(pk,NULL,NULL,&tmp,a,l);
        ...
        switch(topTuple->values[0]){
            ...
            break;//Clique
             ...
             case 71:
                 ...
                 if(compute_G(pk,&add_on_first,&add_on_last,f,h,k,l)==1){
             ...
             case 70:
                 ...
                 if(compute_F(pk,&add_on_first,&add_on_last,c,e,i,j)==1){
             ...
             case 69:
                 ...
                 if(compute_E(pk,&add_on_first,&add_on_last,b,e,i,j)==1){
             ...
             case 68:
                 ...
                 if(compute_D1(pk,&add_on_first,&add_on_last,a,f,i,j)==1){
             ...
             case 67:
                 ...
                 if(compute_C(pk,&add_on_first,&add_on_last,a,f,i,k)==1){
             ...
             case 66:
                 ...
                 if(compute_B(pk,&add_on_first,&add_on_last,a,f,h,k)==1){
             ...
             case 65:
                 ...
                 if(compute_A(pk,&add_on_first,&add_on_last,a,l)==1){
...
void backtrace(pk_compound *pk,bt_struct* bt,int score) {
    ...
    for(int l=0+5;l<=pk->MAX;l++){
        for(int a=0;a<l-5;a++){
            ...
            int mfe=add(mfe1,mfe2);
            int tmp0;
            get_A(pk,NULL,NULL,&tmp0,a,l);
            if(score==add(mfe,tmp0)){
                backtrace_A(pk,bt,tmp0,a,l);
                backtrace_MFEFree(pk,bt,mfe1,0,a-1);
                backtrace_MFEFree(pk,bt,mfe2,l,pk->MAX-1);
                return;
            }
       }
    }
...
int compute_G(pk_compound *pk,Node** first,Node** last,int f,int h,int k,int l) {
    ...
    int possible=1;
    for (int g=f;g<h;g++) {
      if(!evaluate(pk,g,l-1)||!evaluate(pk,h-1,k)){continue;}
      int mfe0 = MFEFree(pk,f,g-1);

      int tmp0;
      if(get_CLIQUE0( pk,first,last,&tmp0,g,h-1,k,l-1)==0){possible=0;}
...
int compute_F(pk_compound *pk,Node** first,Node** last,int c,int e,int i,int j) {
    ...
    for (int d=c+1;d<e+1;d++) {
      if(!evaluate(pk,c,j-1)||!evaluate(pk,d-1,i)){continue;}
      int mfe0 = MFEFree(pk,d,e-1);

      int tmp0;
      if(get_CLIQUE0( pk,first,last,&tmp0,c,d-1,i,j-1)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_E(pk_compound *pk,Node** first,Node** last,int b,int e,int i,int j) {
    ...
    for (int c=b;c<e;c++) {
      ...
      int tmp0;
      if(get_F( pk,first,last,&tmp0,c,e,i,j)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_D1(pk_compound *pk,Node** first,Node** last,int a, int f, int i,int j) {
    ...
    int tmp0;
    if(get_E( pk,first,last,&tmp0,a,f+1,i,j)==0){possible=0;}
    ...
    for(int tmp1=a;tmp1<=min(f-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+2);tmp2<=f;tmp2++){
            if(evaluate(pk,tmp1,tmp2) && tmp1-tmp2+1<=pk->THETA){
                ...
                if(tmp3!=INT_MAX){
                    int tmp;
                    if(get_D1(pk,first,last,&tmp,tmp1+1,tmp2-1,i,j)==0){
                        possible=0;
                    }
                    if(possible==1){
                        //printf("C1 res= %d",tmp);
...
int compute_C(pk_compound *pk,Node** first,Node** last,int a,int f,int i,int k) {
    ...
    for (int j=i+1;j<k+1;j++) {
      ...
      int tmp0;
      if(get_D0( pk,first,last,&tmp0,a,f-1,i,j)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
...
int compute_B(pk_compound *pk,Node** first,Node** last,int a,int f,int h,int k) {
    ...
    for (int i=h;i<k;i++) {
      ...
      int tmp0;
      if(get_C( pk,first,last,&tmp0,a,f,i,k)==0){possible=0;}
      else if(tmp0==INT_MAX){continue;}
    ...
    for (int f=a+3;f<l-2;f++) {
        for (int h=f+1;h<l-1;h++) {
            for (int k=h+1;k<l;k++) {
              ...
              int tmp0;
              if(get_B( pk,first,last,&tmp0,a,f,h,k)==0){possible=0;}
              else if(tmp0==INT_MAX){continue;}
              int tmp1;
              if(get_G( pk,first,last,&tmp1,f,h,k,l)==0){possible=0;}
...
void backtrace_G(pk_compound *pk,bt_struct* bt,int score,int f,int h,int k,int l) {
    ...
    for (int g=f;g<h;g++) {
      ...
      int tmp0;
      get_CLIQUE0( pk,NULL,NULL,&tmp0,g,h-1,k,l-1);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,f,g-1);

        backtrace_CLIQUE0( pk,bt,tmp0,g,h-1,k,l-1);
        bt->bracket+=1;
...
void backtrace_F(pk_compound *pk,bt_struct* bt,int score,int c,int e,int i,int j) {
    ...
    for (int d=c+1;d<e+1;d++) {
      ...
      int tmp0;
      get_CLIQUE0( pk,NULL,NULL,&tmp0,c,d-1,i,j-1);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,d,e-1);

        backtrace_CLIQUE0( pk,bt,tmp0,c,d-1,i,j-1);
        bt->bracket+=1;
...
void backtrace_E(pk_compound *pk,bt_struct* bt,int score,int b,int e,int i,int j) {
    ...
    for (int c=b;c<e;c++) {
      ...
      int tmp0;
      get_F( pk,NULL,NULL,&tmp0,c,e,i,j);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,b,c-1);

        backtrace_F( pk,bt,tmp0,c,e,i,j);
...
void backtrace_D0(pk_compound *pk,bt_struct* bt, int score, int a, int f, int i,int j) {
    ...
    int tmp;
    get_D1(pk,NULL,NULL,&tmp,a+1,f-1,i,j);
    int sc=INTB(pk,a,f,-1,-1);
    if(score==add(sc,tmp)){
        add_pairing(pk,bt,sc,a,f,-1,-1);
        backtrace_D1(pk,bt,tmp,a+1,f-1,i,j);
    }
    return;
}
void backtrace_D1(pk_compound *pk,bt_struct* bt, int score, int a, int f, int i,int j) {
    ...
    int tmp0;
    get_E( pk,NULL,NULL,&tmp0,a,f+1,i,j);
     if(tmp0==INT_MAX){ goto loop;}


    if(score==add(tmp0,0)){
               bt->bracket+=1;
backtrace_E( pk,bt,tmp0,a,f+1,i,j);
    ...
    for(int tmp1=a;tmp1<=min(f-1,INT_MAX);tmp1++){
        for(int tmp2=max(INT_MIN,tmp1+2);tmp2<=f;tmp2++){
            if(evaluate(pk,tmp1,tmp2) && tmp1-tmp2+1<=pk->THETA){
                ...
                if(tmp3!=INT_MAX){
                    int tmp0;
                    get_D1(pk,NULL,NULL,&tmp0,tmp1+1,tmp2-1,i,j);
                    if(score==add(tmp0,tmp3)){
                        add_pairing(pk,bt,tmp3,tmp1,tmp2,a,f);
                        backtrace_D1(pk,bt,tmp0,tmp1+1,tmp2-1,i,j);
                        return;
                    }
                }
            }
...
void backtrace_C(pk_compound *pk,bt_struct* bt,int score,int a,int f,int i,int k) {
    ...
    for (int j=i+1;j<k+1;j++) {
      ...
      int tmp0;
      get_D0( pk,NULL,NULL,&tmp0,a,f-1,i,j);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,j,k-1);

       backtrace_D0( pk,bt,tmp0,a,f-1,i,j);
...
void backtrace_B(pk_compound *pk,bt_struct* bt,int score,int a,int f,int h,int k) {
    ...
    for (int i=h;i<k;i++) {
      ...
      int tmp0;
      get_C( pk,NULL,NULL,&tmp0,a,f,i,k);
      if(tmp0==INT_MAX){continue;}

      if(score==add(tmp0,mfe0)){
        backtrace_MFEFree(pk,bt,mfe0,h,i-1);

        backtrace_C( pk,bt,tmp0,a,f,i,k);
    ...
    for (int f=a+3;f<l-2;f++) {
        for (int h=f+1;h<l-1;h++) {
            for (int k=h+1;k<l;k++) {
              ...
              int tmp0;
              get_B( pk,NULL,NULL,&tmp0,a,f,h,k);
              if(tmp0==INT_MAX){continue;}
              int tmp1;
              get_G( pk,NULL,NULL,&tmp1,f,h,k,l);
              if(tmp1==INT_MAX){continue;}

              if(score==add(add(tmp0,tmp1),0)){

                backtrace_B( pk,bt,tmp0,a,f,h,k);
                backtrace_G( pk,bt,tmp1,f,h,k,l);
