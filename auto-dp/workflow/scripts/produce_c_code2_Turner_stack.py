...
def treat_bag(prev,node):  
    ...
    if tree_dec.bag_type[node]==BagType.CLIQUE:
        ...
        tmp=[]
        first=True
        for line in open('resources/stack_Turner/template_transitional.c').readlines():
        ...
        ','.join([tree_dec.ext_to_letter[i] for i in equation.sorted_indices() if i in tree_dec.ext_to_letter]))
            ...
            if first:
                ...
                declarations.append(line.replace("{",";").replace('0','1').replace("get_","compute_").replace(",int * value",""))
                
            tmp.append(line)
        functions.append("".join(tmp))
        tmp=[]
        first=True
        for line in open('resources/stack_Turner/backtrace/template_transitional.c').readlines():
        ...
        if equation.inward is True:
           ...
           #print("res ",BOUNDI,ADJI,BOUNDJ,ADJJ)
            ...
            with open('resources/stack_Turner/template_diag.c','r') as f:
                line = f.read()
                line = line.replace('MAINNAME',equation.main_name)
                ...
                declarations.append(lp.replace('0','1').replace("get_","compute_").replace(",int * value",""))
                
            with open('resources/stack_Turner/backtrace/template_diag.c','r') as f:
                line = f.read()
                line = line.replace('MAINNAME',equation.main_name)
                line = line.replace('V1',variables[0])
                line = line.replace('V2',variables[1])
           ...
           #print("res ",BOUNDI,ADJI,BOUNDJ,ADJJ)
            ...
            if bag_size[0]<len(const)+4:
                #print("ICIIIII CHG BAGSIZE",len(const)+3,const)
                bag_size[0]=len(const)+4
            with open('resources/stack_Turner/template_diag_reverse.c','r') as f:
                line = f.read()
                line = line.replace('MAINNAME',equation.main_name)
                line = line.replace('V1',variables[0])
                line = line.replace('V2',variables[1])
            ...
            with open('resources/stack_Turner/backtrace/template_diag_reverse.c','r') as f:
                line = f.read()
                line = line.replace('MAINNAME',equation.main_name)
                line = line.replace('V1',variables[0])
                line = line.replace('V2',variables[1])
...
with open('resources/stack_Turner/header_template.c','r') as f:
    header=f.read()
