int fold(HashTable * hashTable) {
    ...
    for(int END=0+ANCH;END<=MAX;END++){
        for(int a=0;a<END-ANCH;a++){
            min_value = min(min_value,compute_ROOT(hashTable,a,END));
        }
    }
    return min_value;
}
...
void backtrace(HashTable * hashTable,int score) {
    for(int END=0+ANCH;END<=MAX;END++){
        for(int a=0;a<END-ANCH;a++){
            ...
            int tmp0=compute_ROOT(hashTable,a,END);
            //printf("h = %d, a=%d, %d+%d %d,%d\n",h,a,mfe2,mfe1,score,compute_A(hashTable,a,h));
            if(score==tmp0){
                backtrace_ROOT(hashTable,tmp0,a,END);
...
int main(int argc, char ** argv) {
    ...
    while(true){
        ...
        int nb_cross=createMatrix(MAX,ss);
        //reading the score
